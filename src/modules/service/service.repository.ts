import AppQueryFind from '../../builder/app-query-find';
import { Service } from './service.model';
import { TService, TServiceDocument } from './service.type';

export const create = async (data: Partial<TService>): Promise<TService> => {
  const result = await Service.create(data);
  return result.toObject();
};

export const findById = async (
  id: string,
): Promise<TServiceDocument | null> => {
  return await Service.findById(id);
};

export const findByIdLean = async (id: string): Promise<TService | null> => {
  return await Service.findById(id).lean();
};

export const findBySlugLean = async (
  slug: string,
): Promise<TService | null> => {
  return await Service.findOne({ slug }).lean();
};

export const findPublic = async (): Promise<TService[]> => {
  return await Service.find({ is_active: true })
    .sort({ order: 1, created_at: -1 })
    .lean();
};

export const findAdminPaginated = async (
  query: Record<string, unknown>,
): Promise<{
  data: TService[];
  meta: { total: number; page: number; limit: number; total_pages: number };
}> => {
  const q = new AppQueryFind(Service, query)
    .search(['title', 'slug', 'description'])
    .filter()
    .sort()
    .paginate()
    .fields()
    .tap((c) => c.lean());

  return await q.execute([
    { key: 'active', filter: { is_active: true } },
    { key: 'inactive', filter: { is_active: false } },
  ]);
};

export const updateById = async (
  id: string,
  payload: Partial<TService>,
): Promise<TServiceDocument | null> => {
  return await Service.findByIdAndUpdate(id, payload, {
    new: true,
    runValidators: true,
  });
};

export const updateOrder = async (
  items: { _id: string; order: number }[],
): Promise<void> => {
  if (!items.length) return;
  const ops = items.map(({ _id, order }) => ({
    updateOne: { filter: { _id }, update: { $set: { order } } },
  }));
  await Service.bulkWrite(ops);
};

export const softDeleteById = async (id: string): Promise<void> => {
  await Service.findByIdAndUpdate(id, {
    is_deleted: true,
    deleted_at: new Date(),
  });
};

export const restoreById = async (
  id: string,
): Promise<TServiceDocument | null> => {
  return await Service.findOneAndUpdate(
    { _id: id, is_deleted: true },
    { is_deleted: false, $unset: { deleted_at: 1 } },
    { new: true },
  ).setOptions({ bypassDeleted: true });
};

export const hardDeleteById = async (id: string): Promise<void> => {
  await Service.findByIdAndDelete(id).setOptions({ bypassDeleted: true });
};
